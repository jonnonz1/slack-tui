...
// Init loads channels on startup.
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	// Delegate updates to focused child
	switch m.focus {
	case FocusSidebar:
		var cmd tea.Cmd
...
func (m Model) View() tea.View {
	...
	// Sidebar
	sidebarView := m.sidebar.View(sidebarWidth, contentHeight+inputHeight, m.focus == FocusSidebar)

	// Messages
	msgView := m.messages.View(mainWidth, contentHeight, m.focus == FocusMessages)

	// Input
	inputView := m.input.View(mainWidth, inputHeight, m.focus == FocusInput, m.activeChannel)

	// Right side: messages + input stacked
	rightPane := lipgloss.JoinVertical(lipgloss.Left, msgView, inputView)

	// Main layout: sidebar | right pane
	mainLayout := lipgloss.JoinHorizontal(lipgloss.Top, sidebarView, rightPane)

	// Status bar
	statusView := m.statusbar.View(m.width)

	v := tea.NewView(lipgloss.JoinVertical(lipgloss.Left, mainLayout, statusView))
	v.AltScreen = true
